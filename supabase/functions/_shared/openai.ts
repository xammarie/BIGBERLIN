// Routes image edits through OpenRouter to avoid OpenAI organisation
// verification. OR exposes the image-capable models (e.g. openai/gpt-5.4-image-2)
// over the chat-completions API with `modalities: ["image","text"]` and
// returns generated images as base64 data URIs in `message.images[]`.
...
export async function editWorksheetImage(
    ...
    if (!apiKey) {
        throw new Error("OPENROUTER_API_KEY is not set on the project");
    }
